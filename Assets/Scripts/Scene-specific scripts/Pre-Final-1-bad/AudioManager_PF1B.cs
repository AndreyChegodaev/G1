...
public class AudioManager_PF1B : MonoBehaviour
{
    public static AudioManager_PF1B instance;
    ...
    {
        audioClips.Remove(audioClips[5]);
        audioClips.Remove(audioClips[5]);
        audioClips.Remove(audioClips[5]);

        if (SaveManager.instance.activeSave.hasFork == true)
        {
            audioClips.Remove(audioClips[3]);
        }
        else audioClips.Remove(audioClips[4]);
    }

    void ExceptionHasNoFinger()
    {
        audioClips.Remove(audioClips[2]);
        audioClips.Remove(audioClips[2]);
        audioClips.Remove(audioClips[2]);
